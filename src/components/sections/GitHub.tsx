...
// NOTE: replace with your real GitHub handle
const USER = "sasi-dev";
...
const stats = [
    { icon: BookMarked, label: "Public Repos", value: "42" },
    { icon: GitCommit, label: "Total Commits", value: "3.2k+" },
    { icon: Star, label: "Stars Earned", value: "1.1k" },
    { icon: GitFork, label: "Forks", value: "180" },
];

const languages = [
    { name: "TypeScript", pct: 38, color: "oklch(0.72 0.18 240)" },
    { name: "Python", pct: 22, color: "oklch(0.78 0.15 200)" },
    { name: "Rust", pct: 14, color: "oklch(0.65 0.20 30)" },
    { name: "Go", pct: 12, color: "oklch(0.75 0.15 190)" },
    { name: "GLSL", pct: 8, color: "oklch(0.7 0.22 320)" },
    { name: "Other", pct: 6, color: "oklch(0.6 0.04 250)" },
];
                    ...
                    <div className="rounded-xl border border-neon/20 bg-card/60 p-8 backdrop-blur-sm lg:col-span-3">
                        ...
                        <div className="mt-6 grid grid-cols-[repeat(26,minmax(0,1fr))] gap-1">
                            {Array.from({ length: 26 * 7 }).map((_, i) => {
                                ...
                                return (
                                    <div
                                        key={i}
                                        className="aspect-square rounded-sm bg-neon"
                                        style={{ opacity: op, boxShadow: op > 0.5 ? "0 0 6px var(--neon)" : "none" }}
                                    />
                                );
                            })}
                        </div>
                        <div className="mt-6 flex items-center justify-between font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
                            ...
                            <div className="flex items-center gap-2">
                                less
                                {[0.1, 0.3, 0.6, 1].map((o) => (
                                    <span key={o} className="h-2.5 w-2.5 rounded-sm bg-neon" style={{ opacity: o }} />
                                ))}
                                more
                            </div>
                        </div>
